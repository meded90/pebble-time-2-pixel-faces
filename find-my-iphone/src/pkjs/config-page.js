...
function reopenNotice(copy) {
  return '<div class="notice">' + copy.reopenInfo + '</div>';
}
...
function pageBody(model, copy) {
  ...
  if (stage === 'devices') {
    ...
    return '<p class="step">' + copy.step3 + '</p><h1>' + copy.which + '</h1>' + error + '<p>' + copy.deviceInfo + '</p><form id="form">' + options +
      '<button' + (devices.length ? '' : ' disabled') + '>' + copy.save + '</button></form><button class="secondary" data-action="refresh">' + copy.refresh + '</button>' + reopenNotice(copy) + privacy(copy);
  }
  if (stage === 'connected') {
    ...
    return '<div class="success">✓</div><h1>' + copy.ready + '</h1>' + error + '<p><b>' + copy.selected.replace('{device}', deviceName) + '</b></p>' +
      ...
      '<button data-action="devices">' + copy.change + '</button><button class="danger" data-action="logout">' + copy.logout + '</button>' + reopenNotice(copy) + privacy(copy);
  }
  return '<p class="step">' + copy.step1 + '</p><h1>' + copy.connect + '</h1>' + error + '<p>' + copy.loginInfo + '</p>' +
    ...
    '<label>' + copy.password + '<input name="password" type="password" autocomplete="current-password" required></label><button>' + copy.continueLabel + '</button></form>' + reopenNotice(copy) + privacy(copy);
}
