...
guard CommandLine.arguments.count == 3 else {
  fputs("Usage: resize-pebble-nearest.swift INPUT OUTPUT\n", stderr)
  exit(2)
}
...
let outputWidth = 200
let outputHeight = 228
let targetAspect = CGFloat(outputWidth) / CGFloat(outputHeight)
let inputAspect = CGFloat(inputImage.width) / CGFloat(inputImage.height)

let cropRect: CGRect
if inputAspect > targetAspect {
  let cropWidth = Int((CGFloat(inputImage.height) * targetAspect).rounded())
  cropRect = CGRect(
    x: (inputImage.width - cropWidth) / 2,
    y: 0,
    width: cropWidth,
    height: inputImage.height
  )
} else {
  let cropHeight = Int((CGFloat(inputImage.width) / targetAspect).rounded())
  cropRect = CGRect(
    x: 0,
    y: (inputImage.height - cropHeight) / 2,
...
print("\(inputImage.width)x\(inputImage.height) -> \(outputWidth)x\(outputHeight), nearest-neighbor, Pebble 64-color palette")
