...
static void update_time(const struct tm *time_value) {
  int hour = time_value->tm_hour;
  ...
  snprintf(s_minute_text, sizeof(s_minute_text), "%02d", time_value->tm_min);
...
static void window_load(Window *window) {
  ...
  s_hour_layer = text_layer_create(GRect(0, 0, 74, 56));
  s_minute_layer = text_layer_create(GRect(0, 47, 74, 56));
  if (!s_hour_layer || !s_minute_layer) {
    return;
  }
