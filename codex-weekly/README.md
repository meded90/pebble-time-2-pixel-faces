...
- расчётная стоимость токенов за текущее окно лимита по той же формуле,
  что использует T3 Code;
...
Локальный bridge запускается на Mac, запрашивает лимиты и дневные счётчики,
    а стоимость читает из локальных файлов T3 Code
`~/.t3/userdata/usage-scan-cache.json` и `usage-model-rates.json`. Он суммирует
только записи Codex внутри активного окна лимита. Токен bridge хранится на
телефоне и не отправляется на часы.
...
Расчёт стоимости требует локальные кэши T3 Code с разбивкой токенов по моделям,
cached input и output. Поэтому точная сумма доступна только через локальный
bridge на Mac. При подключении напрямую к Cloud Run watchface показывает
`-` вместо суммы: метод `account/usage/read` отдаёт дневные общие токены, которых
недостаточно для воспроизведения расчёта T3 Code.
