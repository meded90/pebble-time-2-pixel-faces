...
красный — ошибка соединения или ответа. Рядом с ним выводится краткий статус
сервера: `SET`, `SYNC`, `OK`, либо безопасный код ошибки `AUTH`, `TOKEN`,
`TIME`, `NET`, `DATA` или `ERR`. При ошибке сохранённая квота удаляется:
